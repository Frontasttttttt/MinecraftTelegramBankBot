...
bank_keyboard.add("Курс", "Счёт", "Перевод")
...
def transfer(message):
    ...
    bot.register_next_step_handler(message, check_recipient_or_cancel)
...
def check_recipient_or_cancel(message, markup):
    if message.text.lower() == "отмена":
        bot.send_message(chat_id=message.chat.id, text="Выберите действие", reply_markup=bank_keyboard)
    else:
...
def handle_transfer(message):
    ...
    if len(logins) == 0:
        ...
        bot.send_message(chat_id=message.chat.id, text="Кому бы вы хотели сделать перевод?", reply_markup=markup)
        bot.register_next_step_handler(message, check_recipient_or_cancel, markup)
